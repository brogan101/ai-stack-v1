/**
 * MODEL ORCHESTRATOR — VRAM Guard + Dynamic Intent Router
 * =========================================================
...
import {
  toolsRoot, fetchJson, isWindows,
  ollamaReachable as runtimeOllamaReachable,
} from "./runtime.js";
...
export async function getUniversalGatewayTags(): Promise<GatewayTagsResult> {
  try {
    ...
    return { models: [], ollamaReachable: false, totalSize: 0, totalSizeFormatted: "0 B", totalRunningVram: 0, totalRunningVramFormatted: "0 B", vramGuard };
  }
}
...
function buildCandidateOrder(models: GatewayModel[], roles: Record<string, string>, intent: RouteDecision["intent"], requestedModel?: string): GatewayModel[] {
...
export async function routeModelForMessages(messages: ChatMessage[], requestedModel?: string): Promise<RouteDecision> {
  ...
  const intent        = inferIntentFromMessages(messages);
  const candidates    = buildCandidateOrder(tags.models, roles, intent, requestedModel);
  ...
  const reason   = switched
    ? `Intent router switched from ${requestedModel} to ${selected.name} for ${intent} handling`
    : `Intent router selected ${selected.name} for ${intent} handling`;
  ...
  thoughtLog.publish({ category: "system", title: "Dynamic Route", message: reason, metadata: { intent, requestedModel, selectedModel: selected.name, guardMode: admission.mode, requiredBytes: admission.requiredBytes, availableBytes: admission.availableBytes } });
  return decision;
}
...
export async function streamGatewayChatToSse(response: Response, options: StreamOptions): Promise<void> {
  ...
  const route = await routeModelForMessages(options.messages, options.requestedModel);
...
export async function sendGatewayChat(messages: ChatMessage[], requestedModel?: string): Promise<{model:string;message:string;route:RouteDecision}> {
  const route = await routeModelForMessages(messages, requestedModel);
